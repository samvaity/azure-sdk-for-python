...
class CertificatesClientTest(KeyVaultTestCase):
    ...
    def _validate_certificate_operation(self, pending_cert, vault, cert_name, cert_policy):
        ...
        self.assertEqual(cert_policy.issuer_parameters.name, pending_cert.policy.issuer_name)
    ...
    def _validate_certificate_bundle(self, cert, vault, cert_name, cert_policy):
        ...
        self.assertEqual(cert.policy.secret_properties.content_type, cert_policy.secret_properties.content_type)
        ...
        if cert_policy.x509_certificate_properties:
            _validate_x509_properties(cert.policy, cert_policy.x509_certificate_properties)
        if cert_policy.key_properties:
            _validate_key_properties(cert.policy.key_properties, cert_policy.key_properties)
        if cert_policy.lifetime_actions:
            _validate_lifetime_actions(cert.policy.lifetime_actions, cert_policy.lifetime_actions)

    def _validate_x509_properties(self, cert_bundle_policy, cert_policy_x509_props):
        self.assertIsNotNone(cert_bundle_policy)
        self.assertEqual(cert_policy.x509_certificate_properties.subject,
    ...
    def test_crud_operations(self, vault_client, **kwargs):
        ...
        cert_operation = self.client.create_certificate(cert_name, cert_policy)
        while True:
            pending_cert = self.client.get_certificate_operation(cert_name)
        ...
        secret_bundle = self.client.get_secret(secret_id.name)

        # update certificate
        cert_policy.tags = {'tag1': 'value1'}
        cert_bundle = self.client.update_certificate(cert_id.name, cert_id.version, cert_policy)
        self._validate_certificate_bundle(cert_bundle, client.vault_url, cert_name, cert_policy)

        # delete certificate
        cert_bundle = self.client.delete_certificate(client.vault_url, cert_name)
        self._validate_certificate_bundle(cert_bundle, client.vault_url, cert_name, cert_policy)
